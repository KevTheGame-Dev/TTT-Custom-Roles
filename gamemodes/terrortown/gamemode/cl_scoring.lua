...
function CLSCORE:BuildScorePanel(dpanel)
	...
	for id, s in pairs(scores) do
		if id ~= -1 then
			...
			local role = was_traitor and T("traitor") or (s.was_detective and T("detective") or (s.was_hypnotist and T("hypnotist") or (s.was_mercenary and T("mercenary") or (s.was_jester and T("jester") or (s.was_phantom and T("phantom") or (s.was_glitch and T("glitch") or (s.was_zombie and T("zombie") or (s.was_vampire and T("vampire") or (s.was_swapper and T("swapper") or (s.was_assassin and T("assassin") or (s.was_killer and T("killer") or (s.was_cannibal and T("cannibal") or (s.was_crookedcop and T("crookedcop") or T("innocent"))))))))))))))
...
local wintitle = {
	...
	[WIN_CANNIBAL] = { txt = "hilite_win_cannibal", c = Color(176, 137, 54, 255) }
}
		...
		if id ~= -1 then
			...
			if foundPlayer then
				...
				if role == "inn" or role == "det" or role == "mer" or role == "pha" or role == "gli" then
					...
					countI = countI + 1
				elseif role == "tra" or role == "hyp" or role == "zom" or role == "vam" or role == "ass" or role == "cro" or string.sub(role, 5) == "hyped" or string.sub(role, 5) == "zomed" then
					...
					countT = countT + 1
				elseif role == "jes" or role == "swa" or role == "kil" or role == "can" then
...
function CLSCORE:Init(events)
	...
	for k, e in pairs(events) do
		...
		elseif e.id == EVENT_SELECTED then
			...
			killer = e.killer_ids
			cannibal = e.cannibal_ids
			crookedcop = e.crookedcop_ids
		end
	...
	scores = ScoreEventLog(events, scores, traitors, detectives, hypnotist, mercenary, jester, phantom, glitch, zombie, vampire, swapper, assassin, killer, cannibal, crookedcop)
	...
	self.CannibalIDs = cannibal
	self.CrookedCopIDs = crookedcop
