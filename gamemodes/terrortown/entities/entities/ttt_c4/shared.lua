...
function ENT:UseOverride(activator)
	if IsValid(activator) and activator:IsPlayer() then
		...
		if self:GetArmed() and owner ~= activator and (activator:GetTraitor() or activator:GetHypnotist() or activator:GetVampire() or activator:GetAssassin() or activator:GetZombie() or activator:GetCrookedCop()) and (IsValid(owner) and owner:Alive() and owner:GetTraitor()) then
...
if SERVER then
	...
	local function ReceiveC4Disarm(ply, cmd, args)
		...
		if IsValid(bomb) and bomb:GetClass() == "ttt_c4" and not bomb.DisarmCausedExplosion and bomb:GetArmed() then
			if bomb:GetPos():Distance(ply:GetPos()) > 256 then
				return
			elseif bomb.SafeWires[wire] or ply:IsTraitor() or ply:IsHypnotist() or ply:IsVampire() or ply:IsAssassin() or ply:IsZombie() or ply:IsCrookedCop() or ply == bomb:GetOwner() then
